...
const Auth = {
  ...
  login(identity, password) {
    ...
    const u = db.users.find(
      (x) => (x.username === identity || x.email === identity) && x.password === password
    );
...
const Posts = {
  ...
  flag(id, reason) {
    ...
    if (!p) return;
    p.flags = p.flags || [];
    p.flags.push({ id: uid(), by: me.id, reason: reason || "Inappropriate" });
    saveDB(db);
  },
};
...
function setNavState() {
  ...
  // show/hide admin link if you add one later
}
...
function userLink(username) {
  return `<a href="#/user?u=${encodeURIComponent(username)}" class="user-link">@${escapeHtml(username)}</a>`;
}
    ...
    li.innerHTML = `
      ...
      <ul class="stack" style="margin-top:8px">
        ${(p.comments || [])
          .map(
            (c) =>
              `<li class="card"><span class="meta"><b>${userLink(c.username)}</b> • ${fmt(
                c.ts
              )}</span><div>${escapeHtml(c.text)}</div></li>`
          )
          .join("")}
      </ul>
    `;
...
function renderUserFromHash() {
  ...
  const amIFollowing =
    me && Users.byId(me.id) && Users.byId(me.id).following.includes(u.id);
  ...
  wrap.innerHTML = `
    <div class="card">
      <div class="row" style="align-items:center; gap:12px">
        ...
        <div style="margin-left:auto">
          ${
            me && me.id !== u.id
              ? `<button class="btn" data-follow="${u.id}">${amIFollowing ? "Unfollow" : "Follow"}</button>`
              : ""
          }
        </div>
      </div>
    ...
    <ul class="stack">
      ${posts
        .map(
          (p) =>
            `<li class="post"><div class="meta">${fmt(p.ts)}</div><div>${escapeHtml(
              p.text
            )}</div></li>`
        )
        .join("")}
...
function renderSearch() {
  ...
  const users = Users.list().filter(
    (u) =>
      (u.profile.displayName || "").toLowerCase().includes(q) ||
      (u.username || "").toLowerCase().includes(q)
  );
  const postList = $id("searchPosts"); const userList = $id("searchUsers");
  postList.innerHTML = posts
    .map(
      (p) =>
        `<li class="post"><div class="meta">${userLink(p.username)} • ${fmt(
          p.ts
  ...
  userList.innerHTML = users
    .map(
      (u) =>
        `<li class="card"><div class="row" style="align-items:center;gap:10px">
          <img src="${u.profile.avatar || ""}" alt="" width="36" height="36" style="border-radius:50%;object-fit:cover;background:#222">
          <div><b>${escapeHtml(u.profile.displayName || u.username)}</b><div class="muted">@${escapeHtml(
          u.username
        )}</div></div>
...
function renderAdmin() {
  ...
  $id("flaggedList").innerHTML = flagged
    .map(
      (p) =>
        `<li class="card"><div class="meta">${userLink(p.username)} • ${fmt(
          p.ts
        )}</div><div>${escapeHtml(p.text)}</div>
        <div class="muted">Flags: ${(p.flags || []).length}</div>
        <div class="row"><button class="btn danger" data-admin-del="${p.id}">Delete post</button>
        <button class="btn" data-admin-clear="${p.id}">Clear flags</button></div></li>`
    )
    .join("");

  // users table
  $id("adminUsers").innerHTML = db.users
    .map(
      (u) =>
        `<tr>
...
document.body.addEventListener("click", (e) => {
  ...
  if (followBtn) {
    ...
    if (me.following.includes(targetId)) Users.unfollow?.(targetId) || Users.unfollow(targetId);
